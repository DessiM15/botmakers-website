...
export function BookingWidget() {
  ...
  const fetchSlots = useCallback(async (date: string) => {
    ...
    try {
      const res = await fetch(
        `/api/booking/available-slots?date=${date}&timezone=${timezone}`
      );
      if (res.ok) {
  ...
  function handleTimezoneChange(tz: string) {
    ...
    if (selectedDate) {
      ...
      setSlots([]);
      setSelectedSlot(null);
      fetch(`/api/booking/available-slots?date=${selectedDate}&timezone=${tz}`)
  ...
  if (step === "success") {
    ...
    const displayTime = selectedSlot && selectedDate
    ...
    return (
      <div className="bg-[#0a1929] border border-white/10 rounded-2xl p-8 md:p-12 max-w-2xl mx-auto text-center">
        ...
        {meetingLink && (
          <a
            ...
            className="inline-flex items-center gap-2 px-6 py-3 rounded-lg bg-[#03FF00] text-[#033457] font-semibold hover:bg-[#03FF00]/90 transition-colors mb-4"
          >
  ...
  if (step === "form" && selectedDate && selectedSlot) {
    return (
      <div className="bg-[#0a1929] border border-white/10 rounded-2xl p-6 md:p-8 max-w-lg mx-auto">
  ...
  return (
    <div className="bg-[#0a1929] border border-white/10 rounded-2xl overflow-hidden">
          ...
          <div className="mt-auto">
            ...
            >
              {COMMON_TIMEZONES.map((tz) => (
                <option key={tz.value} value={tz.value} className="bg-[#0a1929]">
                  {tz.label}
                </option>
              ))}
            </select>
